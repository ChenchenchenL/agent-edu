...
class KnowledgeMemoryStatusUpdate:
    ...
    assessment_evidence_count: int | None = None
    task_evidence_count: int | None = None
    validation_status: str | None = None
    promotion_rationale: str | None | MemoryFieldUnset = MemoryFieldUnset.UNSET

    @classmethod
    def from_legacy_kwargs(cls, values: dict[str, object]) -> "KnowledgeMemoryStatusUpdate":
        allowed = set(cls.__dataclass_fields__)
        unknown = sorted(set(values) - allowed)
        if unknown:
            raise ValidationError(f"Unsupported knowledge memory status update fields: {', '.join(unknown)}.")
...
class KnowledgeMemory:
    ...
    task_evidence_count: int = 0
    semantic_category: str = "concept"
    ...
    def build(
        ...
        *,
        ...
        if knowledge_level not in KNOWLEDGE_LEVELS:
            raise ValidationError("Unsupported knowledge memory level.")
        ...
        return cls(
            ...
            prerequisite_weight=1.0 if knowledge_level in {"foundation", "core"} else 0.5,
            assessment_evidence_count=0,
            task_evidence_count=0,
            semantic_category="concept" if knowledge_level != "foundation" else "prerequisite",
    ...
    def with_status(
        self,
        status: str,
        update: KnowledgeMemoryStatusUpdate | None = None,
        **legacy_updates: object,
        ...
        status_update = update or KnowledgeMemoryStatusUpdate.from_legacy_kwargs(legacy_updates)
        ...
        return KnowledgeMemory(
            ...
            prerequisite_keys=self.prerequisite_keys,
            ...
            assessment_evidence_count=self.assessment_evidence_count
            if status_update.assessment_evidence_count is None
            else status_update.assessment_evidence_count,
            task_evidence_count=self.task_evidence_count
            if status_update.task_evidence_count is None
            else status_update.task_evidence_count,
    ...
    def with_compression(self, *, compressed_into_id: str) -> "KnowledgeMemory":
        return KnowledgeMemory(
            id=self.id,
            learner_profile_id=self.learner_profile_id,
            learner_goal_id=self.learner_goal_id,
...
@dataclass(frozen=True)
class KnowledgeMemoryEmbeddingRecord:
    id: str
    memory_id: str
    learner_profile_id: str
    learner_goal_id: str | None
...
@dataclass(frozen=True)
class KnowledgeMemoryRetrievalResult:
    memories: list[RetrievedKnowledgeMemory]
    provider: str | None
    model: str | None
    latency_ms: int
    candidate_count: int
